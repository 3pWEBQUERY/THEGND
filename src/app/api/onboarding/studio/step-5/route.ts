...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!user || user.userType !== 'STUDIO') {
      return NextResponse.json(
        { error: 'Nur Studios können dieses Onboarding verwenden' },
        { status: 403 }
      )
    }
    ...
    console.error('Studio step 5 onboarding GET error:', error)
    return NextResponse.json(
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('Studio step 5 onboarding error:', error)
    return NextResponse.json(
      { error: 'Server Fehler beim Onboarding' },
      { status: 500 }
    )
