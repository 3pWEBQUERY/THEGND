...
export default function EscortOnboardingStep7() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      try {
        const res = await fetch('/api/onboarding/escort/step-7')
        if (!res.ok) return
  ...
  function initMap(lat: number, lng: number) {
    ...
    // Use new Maps JS v3.55+ features if available
    ...
    if (usesAdvancedMarker) {
      ...
      // Fallback to classic Marker if AdvancedMarker not present
      // @ts-ignore
      const m = new g.maps.Marker({ position: center, map: mapRef.current })
      // no ref kept for classic marker to keep it simple
    }
  }
        ...
        <div className="max-w-7xl mx-auto px-6 py-6">
          <div className="flex justify-between items-center">
            ...
            <div className="text-sm font-light tracking-widest text-gray-600">
              ESCORT-EINRICHTUNG – SCHRITT 7/7
            </div>
          </div>
        </div>
      </nav>
              ...
              <div>
                ...
                <Input
                  ...
                  ref={(el) => {
                    ...
                    // react-hook-form's ref handling
                    // @ts-ignore
