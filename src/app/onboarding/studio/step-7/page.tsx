...
export default function StudioOnboardingStep7() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const res = await fetch('/api/onboarding/studio/step-7')
        if (!res.ok) return
  ...
  function initMap(lat: number, lng: number) {
    ...
    if (!mapRef.current) {
      mapRef.current = new g.maps.Map(mapContainerRef.current, { center, zoom: 14, mapId: 'onboarding-studio-step7', disableDefaultUI: true })
    } else {
  ...
  async function onSubmit(values: Step7Form) {
    ...
    setError('')
    try {
      const res = await fetch('/api/onboarding/studio/step-7', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(values) })
      if (!res.ok) {
    ...
    <div className="min-h-screen bg-white">
      ...
      <nav className="absolute top-0 w-full z-50 bg-transparent">
        <div className="max-w-7xl mx-auto px-6 py-6">
          <div className="flex justify-between items-center">
            ...
            <div className="text-sm font-light tracking-widest text-gray-600">STUDIO-EINRICHTUNG – SCHRITT 7/7</div>
          </div>
        </div>
      </nav>
      ...
      <div className="min-h-screen flex items-center justify-center px-6 pt-20">
        <div className="w-full max-w-2xl">
          ...
          <form id="studio-step7-form" onSubmit={handleSubmit(onSubmit)} className="p-6 border bg-gray-50 text-sm text-gray-700">
          ...
          <div className="flex flex-col sm:flex-row gap-6 pt-8">
            ...
            <Button type="submit" form="studio-step7-form" disabled={isLoading} className="flex-1 bg-green-500 hover:bg-green-600 disabled:opacity-50 text-white font-light tracking-widest py-4 text-sm uppercase rounded-none">{isLoading ? 'Speichern …' : 'Fertig & Zur Übersicht'}</Button>
          </div>
        </div>
      </div>
    </div>
